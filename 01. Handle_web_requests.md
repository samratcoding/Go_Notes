...
## Using `http.NewRequest` for Post Request
[Return Table of Content](#Table-of-Contents)
...
// DynamicData for unknown JSON format using `interface{}` (can be `map[string]interface{}`)
...
func sendJSONWithEncoder() {
	...
	// Dynamic JSON data (unknown structure)
	data := DynamicData{
