...
This document summarizes different ways to make HTTP requests in Go, including `GET`, `POST`, `PUT`, `DELETE`, and handling authentication, cookies, and sessions.
...
❌ No authentication or advanced settings.

---

## 2. Using `http.NewRequest` + `http.Client{}` (For All HTTP Methods)
...
```go
requestBody, _ := json.Marshal(map[string]string{"name": "John"})
...
response, err := client.Do(req)
```
✅ Required for APIs with authentication.

---

## 3. Handling Cookies & Sessions
...
response, err := client.Do(req)
```
✅ Used when maintaining a session.
...
## 5. Comparing HTTP Methods

| Method  | Purpose | Example |
|---------|---------|---------|
| **GET** | Retrieve data | `http.NewRequest("GET", url, nil)` |
| **POST** | Send data | `http.Post(url, "application/json", bytes.NewBuffer(requestBody))` |
| **PUT** | Update resource | `http.NewRequest("PUT", url, bytes.NewBuffer(requestBody))` |
| **DELETE** | Remove resource | `http.NewRequest("DELETE", url, nil)` |

---

## 6. Conclusion
- **Use `http.Post`** for simple JSON requests without custom headers.
- **Use `http.NewRequest`** for `GET`, `PUT`, `DELETE`, and requests requiring authentication.
- **Handle cookies properly** when maintaining sessions.
